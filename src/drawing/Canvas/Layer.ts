...
export class Layer {
    ...
    addBrushStroke(stroke: PIXI.Container) {
        const strokeSprite = renderAsSprite(stroke, app.renderTexturePool.acquire(this.canvas.settings.width, this.canvas.settings.height))
    ...
    test() {
        // const image = app.application.renderer.plugins.extract.image(this.container.mask['texture'])
